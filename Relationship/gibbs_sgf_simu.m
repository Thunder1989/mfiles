function [Y, Z, M, Q, R] = gibbs_sgf_simu(Q_ahu, R_ahu, F_switch)
    ...
    %         tmp = mvnrnd( F*[y_prev vy_prev]',  Q{state(i)} );
            Y = [Y; tmp_y];
            tmp_x = mvnrnd( H*[y_prev vy_prev]', R_star{state(i)} );
            X = [X; tmp_x];
            y_prev = tmp_y(1);
            vy_prev = tmp_y(2);
        end
...
%     fprintf('initial Q and R:\n');
    ...
    figure
    subplot(3,1,1)
    plot(Z_err, 'k-', 'LineWidth', 2)
    subplot(3,1,2)
    plot(Q_err, 'LineWidth', 2)
    subplot(3,1,3)
    plot(R_err, 'LineWidth', 2)
